  ...
  var PageableCollection = Backbone.PageableCollection = Backbone.Collection.extend({
    ...
    */
    _checkState: function (state) {
      ...
      if (totalRecords != null && pageSize != null && currentPage != null &&
        ...
        } else if (mode != "timeline" && (currentPage < firstPage ||
                 (totalPages > 0 &&
    ...
    */
    switchMode: function (mode, options) {
      ...
      if (!_contains(["server", "client", "infinite", "timeline"], mode)) {
        throw new TypeError('`mode` must be one of "server", "client", "infinite" or "timeline"');
      }
      ...
      if (mode != "server" && mode != "timeline" && !fullCollection) {
      ...
      else if (mode == "timeline" && !fullCollection) {
        fullCollection = this._makeFullCollection(options.models || [], options);
        fullCollection.pageableCollection = this;
        this.fullCollection = fullCollection;
        fullCollection.comparator = this._fullComparator;
    ...
    */
    hasPreviousPage: function () {
      var state = this.state;
      var currentPage = state.currentPage;
      if (this.mode == "timeline") return true;
    ...
    */
    getPage: function (index, options) {
      ...
      if (mode == "timeline") {
        options.from = this.first() ? this.first().get(this.queryParams.cursorId) : null;
        options.to = this.last() ? this.last().get(this.queryParams.cursorId) : null;
      }
      ...
      var pageModels = fullCollection && fullCollection.length ?
        ...
        [];
      if ((mode == "client" || (mode == "timeline" && !_isEmpty(pageModels)) ||
           (mode == "infinite" && !_isEmpty(pageModels))) &&
          !options.fetch) {
    ...
    fetch: function (options) {
      ...
      if (mode == "timeline") {
        if(this.pageDirection == "next") {
          options.data[options.data.maxId] = this.reversed ? options.from : options.to;
        } else if(this.pageDirection == "prev") {
          options.data[options.data.sinceId] = this.reversed ? options.to : options.from;
        }
        delete options.data.maxId;
        delete options.data.sinceId;
        delete options.data.cursorId;
